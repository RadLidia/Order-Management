...
/**
 * Generic class that contains the methods for performing the basic queries, 
 * on the specific tabel from the database.
 * 
...
public class AbstractDAO<T> {
	...
	/**
	 * 
	 * Method used to create the select query: "SELECT * FROM warehouse.table WHERE field =?"
	 * 
	...
	/**
	 * 
	 * Method used to create the select all query: "SELECT * FROM warehouse.table"
	 * 
     ...
     */
	public T findByIsCahier(int flag) {
		Connection connection = null;
		PreparedStatement statement = null;
		ResultSet resultSet = null;
		String query = createSelectQuery("isCashier");
		try {
			connection = ConnectionWarehouse.getConnection();
			statement = connection.prepareStatement(query);
	...
	public int update(String name, int quantity) {
		...
		try {
			...
			ConnectionWarehouse.close(updateStatement);
		}
		return updatedId;
	}
